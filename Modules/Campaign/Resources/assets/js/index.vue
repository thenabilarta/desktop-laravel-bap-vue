      ...
      <sui-table selectable celled v-else>
        <sui-table-header>
          <sui-table-row>
            ...
            <sui-table-header-cell>Number of Layouts</sui-table-header-cell>
            <sui-table-header-cell>Total Duration</sui-table-header-cell>
        ...
        <sui-table-body>
          <sui-table-row v-for="(t, index) in tableList" :key="index">
            <sui-table-cell>{{ t.campaign }}</sui-table-cell>
            <sui-table-cell>{{ t.NumberLayouts }}</sui-table-cell>
            <sui-table-cell>{{ t.totalDuration }}</sui-table-cell>
...
export default {
  ...
  mounted() {
    axios.get("http://127.0.0.1:8000/campaign/data").then((res) => {
