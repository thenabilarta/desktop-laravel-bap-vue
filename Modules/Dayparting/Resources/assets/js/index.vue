      ...
      <sui-table selectable celled v-else>
        <sui-table-header>
          <sui-table-row>
            ...
            <sui-table-header-cell>Start Time</sui-table-header-cell>
            <sui-table-header-cell>End Time</sui-table-header-cell>
        ...
        <sui-table-body>
          <sui-table-row v-for="(t, index) in tableList" :key="index">
            <sui-table-cell>{{ t.name }}</sui-table-cell>
            <sui-table-cell>{{
            ...
            <sui-table-cell>{{ t.endTime ? t.endTime : "--" }}</sui-table-cell>
...
export default {
  ...
  mounted() {
    axios.get("http://127.0.0.1:8000/dayparting/data").then((res) => {
